...
def probe_emergency_detection() -> ProbeResult:
    ...
    try:
        ...
        cases = [
            ...
            ("Sangrado masivo que no se detiene", True),
...
def probe_knowledge_base() -> ProbeResult:
    ...
    t0 = time.monotonic()
    try:
        from knowledge.icd10_catalog import ICD10Catalog

        catalog = ICD10Catalog()
        conditions = catalog.get_all_conditions()
        assert conditions is not None, "get_all_conditions() returned None"
        count = len(conditions)
        assert count > 100, f"Expected >100 conditions, got {count}"

        # Verify structure
        sample = conditions[0]
        assert hasattr(sample, "icd10_code"), "Missing icd10_code"
        assert hasattr(sample, "name"), "Missing name"
...
def write_report(report: CanaryReport, out_dir: Path) -> None:
    ...
    md_lines.extend(["", "---", f"*Generated by MedX Canary v0.1.0*", ""])
